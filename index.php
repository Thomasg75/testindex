...
<div id="container">
    <div id="message_tchat" style='overflow: scroll; overflow-x: hidden; max-height: 500px;'>
        ...
        $resultat = $pdo->query("SELECT d.id_dialogue, m.pseudo, m.civilite, d.message, date_format(d.date, '%d/%m/%Y') as datefr, date_format(d.date, '%H:%i:%s') as heurefr FROM dialogue d, membre m WHERE m.id_membre = d.id_membre ORDER BY d.date DESC"); // permet de prendre les 15 derniers commentaires dans le bon ordre
        ...
        <div id="liste_membre_connecte">
            <?php
                echo '<h2>Membre connectés : </h2>';
                $resultat = $pdo->query("SELECT * FROM membre WHERE date_connexion=" . (time()-3600)); // Affiche membres s'étant connecté ou ayant posté un message dans la dernière heure (3600 secondes)
        ...
        <div id="formulaire_tchat">
                ...
                <form action="#" method="post">
                    <textarea name="message" id="message" cols="30" rows="10" value="<?php
